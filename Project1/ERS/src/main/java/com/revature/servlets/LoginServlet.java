...
public class LoginServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//		+ username);
		UserDaoImpl tempUser = new UserDaoImpl();
		
		//TODO setting pop
		User localUser = tempUser.getUser(username, password);
		
		if(localUser == null)
		{
			//TODO
			//response redirect to frontpage (optionally with error message)
		}
		else
		{
			if(localUser.getRole() == UserRole.MANAGER)
			{
			...
			//{
			// SESSION
			//dispatcher to specific html page
			// redirect to manager page	via response
			//}
			
			//after page then ajax
		}
